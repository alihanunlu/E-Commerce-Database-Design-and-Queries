...
select * from urun where urunAdi like '%pirinç%'
...
select urunAdi as "Ürün Adý",fiyat as "Fiyat" from urun
...
SELECT * ,DATEDIFF(YEAR,dogumTarihi ,GETDATE()) AS YAÞ from kullanici
...
select distinct miktar,count(miktar) as MiktarSayýsý from siparisDetay group by miktar
SELECT CONVERT(VARCHAR(5),GETDATE(),108)
select COUNT(*) as sayýsý, altkategorino from urun group by altkategorino
select COUNT(*) as sayýsý, altkategorino from urun group by altkategorino order by altkategorino desc
select altkategorino,count(urunNo) as sayýsý from urun group by altkategorino having count(urunNo) >=1000
SELECT GETDATE()
select cast( avg(fiyat) as decimal (35,2)) as 'ort' from urun
select * from sepet where kullaniciNo=48
select urunNo,count(*) as sayýsý from sepet group by urunNo 
...
select sehirNo,COUNT(ilceNo) as ilcesayýsý from ilce group by sehirNo 
select sehirNo,COUNT(ilceNo) as ilcesayýsý from ilce group by sehirNo order by COUNT(ilceNo) desc
select sehirNo,COUNT(ilceNo) as ilcesayýsý from ilce  group by sehirNo having  COUNT(ilceNo)>20
select  (avg( DATEDIFF(YEAR,dogumTarihi,GETDATE()) )) as YASortalama from kullanici 
select kategorino,COUNT(altkategorino) as altkatagorisayýsý from urunAltKateg group by kategorino 
select kategorino,COUNT(altkategorino) as altkatagorisayýsý from urunAltKateg group by kategorino having COUNT(altkategorino)>=7
